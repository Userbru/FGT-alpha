...
class MainActivity : ComponentActivity() {
        ...
        setContent {
            ...
            Surface {
                Column(
                    ...
                    Text("1) Ative o teclado nas Configurações. 2) Ative a Acessibilidade (filtrar teclas).")
                    OutlinedTextField(
                        value = trigger,
                        onValueChange = { trigger = it },
                        label = { Text("Gatilho (ex: START+Y)") }
                    )
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button(onClick = { recording.clear(); last = now() }) { Text("Gravar macro") }
                        Button(onClick = {
                            MacroStore.record(trigger, recording.toList())
                            recording.clear()
                        }) { Text("Salvar") }
                    }
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        // grava eventos quando você aperta botões do controle DENTRO do app
